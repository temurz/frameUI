//
//  PurpleThemeColors.swift
//  Tedr
//
//  Created by Kostya Lee on 24/06/25.
//

import Foundation
import UIKit
extension PurpleTheme {
    var backgroundColor: UIColor {
        return UIColor(hex: "#000000")
    }
    var backgroundPrimaryColor: UIColor {
        return UIColor(hex: "#3F135C")
    }
    var backgroundSecondaryColor: UIColor {
        return UIColor(hex: "#6443F3")
    }
    
    
    var bgSecondaryTransparent20: UIColor {
        return UIColor(hex: "#00000033")
    }
    var backgroundTertiaryColor: UIColor {
        return UIColor(hex: "#403378")
    }
    var darkTextColor: UIColor {
        return UIColor(hex: "#FFFFFF")
    }
    var subTextColor: UIColor {
        return UIColor(hex: "#FFFFFF").withAlphaComponent(0.8)
    }
    var whiteColor: UIColor {
        return UIColor(hex: "FFFFFF")
    }
    var orangeColor: UIColor {
        return UIColor(hex: "#F88633")
    }
    var addDarkTextColor: UIColor {
        return UIColor(hex: "#FFFFFF").withAlphaComponent(0.6)
    }
    var addSubTextColor: UIColor {
        return UIColor(hex: "#FFFFFF").withAlphaComponent(0.5)
    }
    var systemBlueColor: UIColor {
        return UIColor(hex: "#329AFB")
    }
    var systemRedColor: UIColor {
        return UIColor(hex: "#F45047")
    }
    var systemYellowColor: UIColor {
        return UIColor(hex: "#FFCF4D")
    }
    var systemGreenColor: UIColor {
        return UIColor(hex: "#51CE92")
    }
    var statusProcessingColor: UIColor {
        return UIColor(hex: "#329AFB")
    }
    var statusClosedColor: UIColor {
        return UIColor(hex: "#323C58")
    }
    var statusDeliveredColor: UIColor {
        return UIColor(hex: "#51CE92")
    }
    var statusReturnedColor: UIColor {
        return UIColor(hex: "#F45047")
    }
    var iconColor: UIColor {
        return subTextColor
    }
    var contentPrimary: UIColor {
        return UIColor(hex: "#EEE5F8")
    }
    var contentSecondary: UIColor {
        return UIColor(hex: "#A190CC")
    }
    var contentWhite: UIColor {
        return UIColor(hex: "#FFFFFF")
    }
    var contentTurquoise: UIColor {
        return UIColor(hex: "#1BCAB4")
    }
    var contentPink: UIColor {
        return UIColor(hex: "#EF46D9")
    }
    var contentBlue: UIColor {
        return UIColor(hex: "#4389FF")
    }
    var contentRed: UIColor {
        return UIColor(hex: "#F43C5D")
    }
    var contentYellow: UIColor {
        return UIColor(hex: "#FAC765")
    }
    
    var contentPurple : UIColor {
        return UIColor(hex: "#8D5FF0")
    }
    
    var reorderActionBackground: UIColor {
        return UIColor(hex: "#51DDFC33") // ChatListController table action
    }
    var pinActionBackground: UIColor {
        return UIColor(hex: "#AD6CFF33") // ChatListController table action
    }
    var readActionBackground: UIColor {
        return UIColor(hex: "#4389FF33") // ChatListController table action
    }
    var trashActionBackground: UIColor {
        return UIColor(hex: "#EE627B33") // ChatListController table action
    }
    var muteActionBackground: UIColor {
        return UIColor(hex: "#FAC76533") // ChatListController table action
    }
    var pinkGradientUpColor: UIColor {
        return UIColor(hex: "#FB94ED")
    }
    var pinkGradientDownColor: UIColor {
        return UIColor(hex: "#EF46D9")
    }
    var blueGradientUpColor: UIColor {
        return UIColor(hex: "#A895FF")
    }
    var blueGradientDownColor: UIColor {
        return UIColor(hex: "#6F56DE")
    }
    var purpleBackgroundColor: UIColor {
        return UIColor(hex: "#3A1659")
    }
    var buttonTurquoiseColor: UIColor { // бирюзовый
        return UIColor(hex: "#38AEC7")
    }
    
    var tabbarUnionBackgroundColor: UIColor {
        return UIColor(hex: "#6443F3")
    }
    var textFieldBackgroundColor: UIColor {
        return UIColor(hex: "#000000").withAlphaComponent(0.3)
    }
    var ethereumNetworkColor: UIColor {
        return UIColor(hex: "#41DBA3")
    }
    var tronNetworkColor: UIColor {
        return UIColor(hex: "#B73E3E")
    }
    var bitcoinNetworkColor: UIColor {
        return UIColor(hex: "#F39321")
    }
    var tonNetworkColor: UIColor {
        return UIColor(hex: "#2E67D2")
    }
    var fiatNetworkColor: UIColor {
        return UIColor(hex: "#62D970")
    }
    var eosNetworkColor: UIColor {
        return UIColor(hex: "#3567E2")
    }
    var bscNetworkColor: UIColor {
        return UIColor(hex: "#D0BB00")
    }
    var bgWhiteTransparent10: UIColor {
        return UIColor(hex: "#FFFFFF1A")
    }
    var buttonQuaternaryTransparentDefaultContent: UIColor {
        return UIColor(hex: "#FFFFFF")
    }
    var buttonQuaternaryTransparentDefaultBg: UIColor {
        return UIColor(hex: "#FFFFFF").withAlphaComponent(0.2)
    }
    var buttonSecondaryPinkDisabledContent: UIColor {
        return UIColor(hex: "#FFFFFF66")
    }
    var buttonSecondaryPinkDisabledBg: UIColor {
        return UIColor(hex: "#0000004D")
    }
    var bgBlackTransparent20: UIColor {
        return UIColor(hex: "#00000033")
    }
    var chatBubbleOutcomingColor: UIColor {
        return UIColor(hex: "#5C2AC7")
    }
    var chatBubbleIncomingColor: UIColor {
        return UIColor(hex: "#120828")
    }
    
    var primaryBlueDefaultBg : UIColor {
        return UIColor(hex: "#3B72F2")
    }
}
